...
while true; do
  ...
  if ready_nodes=$(kubectl --kubeconfig=/root/.kube/config get nodes -o json | jq -r '.items[] | select((.status.conditions[] | select(.type=="Ready" and .status=="True")) and (.spec.taints == null or (.spec.taints | map(select(.key=="node-role.kubernetes.io/control-plane" and .effect == "NoSchedule")) | length == 0))) | .metadata.name'); then
